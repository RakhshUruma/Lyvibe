...
export const PRESETS: Record<string, Mood> = {
  cyber: base({
    ...
    rgbSplit: 4, glitchRate: 0.35, ghostRate: 0.45, motionRate: 0.7, bgSwapRate: 0.45,
    ...
    bgCssVariants: [
      "background: #02030a; background-image: linear-gradient(0deg,transparent 59px,rgba(0,240,255,0.22) 60px),linear-gradient(90deg,transparent 59px,rgba(0,240,255,0.22) 60px); background-size: 60px 60px; animation: vjBgScan 6s linear infinite;",
      "background: radial-gradient(ellipse at center, #0a1530, #02030a 70%); animation: vjBgPulse 3.6s ease-in-out infinite;",
      "background: #02030a; background-image: repeating-linear-gradient(135deg, transparent 0 40px, rgba(255,45,160,0.10) 40px 41px); animation: vjBgScan 4s linear infinite;",
    ],
  }),

  dream: base({
    bg: "#1a0e2a", accent: "#ffb3e6", hot: "#ffd1ff", cool: "#a0e5ff",
    blur: 0.5, rgbSplit: 1, glitchRate: 0.05, ghostRate: 0.55, motionRate: 0.85, bgSwapRate: 0.5,
    tilt: 3, positionMode: "scatter", color: "#fff5fc", weight: 400, font: "Georgia, serif",
  ...
  neon: base({
    ...
    rgbSplit: 5, glitchRate: 0.25, ghostRate: 0.55, motionRate: 0.8, bgSwapRate: 0.5,
    tilt: 8, positionMode: "scatter",
    entryKeyframes: k("popIn", "spin", "drop", "slideRight"),
    motionKeyframes: m("bounce", "pulseGlow", "sway"),
    bgKeyframes: bg("pulse", "spin"),
    bgCssVariants: [
      "background: radial-gradient(ellipse at center, #2a0044, #0a0014 70%); animation: vjBgPulse 2.4s ease-in-out infinite;",
      "background: linear-gradient(45deg, #0a0014, #1a0030); background-size: 200% 200%; animation: vjBgDrift 8s ease-in-out infinite alternate;",
      "background: #0a0014; background-image: radial-gradient(circle at 20% 80%, rgba(255,95,160,0.32), transparent 40%), radial-gradient(circle at 80% 20%, rgba(157,58,255,0.32), transparent 40%); animation: vjBgPulse 1.8s ease-in-out infinite;",
      "background: conic-gradient(from 0deg at 50% 50%, #1a0030, #0a0014, #2a0044, #0a0014, #1a0030); animation: vjBgSpin 30s linear infinite;",
    ],
  }),

  vhs: base({
    bg: "#080806", accent: "#f4d03f", hot: "#ff5040", cool: "#40d0ff",
    rgbSplit: 6, glitchRate: 0.55, ghostRate: 0.4, motionRate: 0.7, bgSwapRate: 0.75,
    tilt: 0, positionMode: "center",
    entryKeyframes: k("glitchAppear", "slam", "slideLeft"),
    motionKeyframes: m("flicker", "jitter"),
    bgKeyframes: bg("scan", "flicker"),
    bgCssVariants: [
      "background: #080806; background-image: repeating-linear-gradient(0deg, rgba(255,255,255,0.05) 0 2px, transparent 2px 4px); animation: vjBgScan 4s linear infinite, vjBgFlicker 0.18s steps(4) infinite;",
      "background: #080806; background-image: linear-gradient(180deg, rgba(255,255,255,0.06), transparent 30%), repeating-linear-gradient(0deg, rgba(255,255,255,0.04) 0 1px, transparent 1px 3px); animation: vjBgScan 8s linear infinite;",
      "background: #080806; background-image: radial-gradient(ellipse at 50% 50%, rgba(244,208,63,0.10), transparent 60%); animation: vjBgFlicker 0.18s steps(4) infinite;",
    ],
  }),
  ...
  chaos: base({
    ...
    rgbSplit: 8, blur: 1, glitchRate: 0.7, ghostRate: 0.85, motionRate: 0.95, bgSwapRate: 0.85,
    tilt: 18, positionMode: "scatter",
    ...
    bgCssVariants: [
      "background: linear-gradient(45deg, #ff00ff, #00ffff, #ffff00, #ff00ff); background-size: 400% 400%; animation: vjBgDrift 4s ease-in-out infinite alternate, vjBgFlicker 0.18s steps(4) infinite;",
      "background: conic-gradient(from 0deg, #ff00ff, #00ffff, #ffff00, #ff00ff); animation: vjBgSpin 6s linear infinite;",
      "background: radial-gradient(circle at 50% 50%, #ff0080, #000); animation: vjBgPulse 0.6s ease-in-out infinite;",
      "background: repeating-linear-gradient(45deg, #ff00ff 0 30px, #00ffff 30px 60px, #ffff00 60px 90px); background-size: 200% 200%; animation: vjBgDrift 1.6s ease-in-out infinite alternate;",
    ],
  }),
};
